...
income_precaution_prop <- earthquake %>%
                          group_by(household_income, taken_preacautions) %>%
                          summarise(count = n()) %>%
                          mutate(proportion = count / sum(count))
...
exp_precaution_prop <- earthquake %>%
                        group_by(experienced_earthquake, taken_preacautions) %>%
                        summarise(count = n()) %>%
                        mutate(proportion = count / sum(count))
...
income_exp_precaution_prop <- earthquake %>%
                          group_by(household_income, experienced_earthquake,
                          taken_preacautions) %>%
                          summarise(count = n()) %>%
                          mutate(proportion = count / sum(count))
...
missing_rows <- data.frame(household_income = c("$100,000 to $124,999", "$125,000 to $149,999", "$175,000 to $199,999", "$175,000 to $199,999"), experienced_earthquake = c("No", "No", "No", "Yes, one or more major ones"), taken_preacautions = c("Yes", "Yes", "Yes", "Yes"), count = c(0, 0, 0, 0), proportion = c(0.00, 0.00, 0.00, 0.00))

income_exp_precaution_props <- na.omit(income_exp_precaution_prop)
income_exp_precaution_prop_bind <- bind_rows(income_exp_precaution_props, missing_rows)
...
ggplot(income_exp_precaution_prop_bind %>% filter(taken_preacautions == "Yes"), aes(x = household_income, 
    y = proportion, 
  ...
  labs(x = "Household Income", 
       ...
       title = "Proportion of taking precautions across income levels and experience with earthquakes.") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
